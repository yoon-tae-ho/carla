...
# Unreal's bundled clang does not always discover the host libstdc++
# include directories on newer Linux distributions.
foreach(_CARLA_STDCPP_VERSION 12 11 10 9)
  if(EXISTS "/usr/include/c++/\${_CARLA_STDCPP_VERSION}")
    set(CMAKE_CXX_FLAGS "\${CMAKE_CXX_FLAGS} -isystem /usr/include/c++/\${_CARLA_STDCPP_VERSION}" CACHE STRING "" FORCE)
  endif()
  if(EXISTS "/usr/include/x86_64-linux-gnu/c++/\${_CARLA_STDCPP_VERSION}")
    set(CMAKE_CXX_FLAGS "\${CMAKE_CXX_FLAGS} -isystem /usr/include/x86_64-linux-gnu/c++/\${_CARLA_STDCPP_VERSION}" CACHE STRING "" FORCE)
  endif()
  if(EXISTS "/usr/lib/gcc/x86_64-linux-gnu/\${_CARLA_STDCPP_VERSION}/libstdc++.so")
    set(CMAKE_CXX_FLAGS "\${CMAKE_CXX_FLAGS} -L/usr/lib/gcc/x86_64-linux-gnu/\${_CARLA_STDCPP_VERSION}" CACHE STRING "" FORCE)
    set(CMAKE_EXE_LINKER_FLAGS "\${CMAKE_EXE_LINKER_FLAGS} -L/usr/lib/gcc/x86_64-linux-gnu/\${_CARLA_STDCPP_VERSION}" CACHE STRING "" FORCE)
    set(CMAKE_SHARED_LINKER_FLAGS "\${CMAKE_SHARED_LINKER_FLAGS} -L/usr/lib/gcc/x86_64-linux-gnu/\${_CARLA_STDCPP_VERSION}" CACHE STRING "" FORCE)
  endif()
endforeach()
